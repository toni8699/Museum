...
No collision/navmesh · synthesized look path only · fixed-eye free-look only at Paris stop · selective shadows/GLTF assets only in Paris · no exhibit interaction · no spatial audio · no tangent handles · no per-edge timing · no guided-order editing · no node/connection deletion · `targetWaypoints` typed but unused · `lockInteraction` unused in checked-in data · mobile hides route list · curves can clip walls if anchors are wrong.

...
**Status:** Active release authority. Phases 1–3 complete (through 3.6). Complete-refactor slices 1–9.5 landed. **Phase 4 complete. Phase 5.1 complete. Phase 5.2 complete. Phase 5.3 complete. Phase 5.4 complete. Phase 5 shipped: binary upload + object URLs + package export landed and gated (791 full tests, check 0/0, build green, visitor chunk isolation verified, browser smoke clean, polish fixes applied).** One open cosmetic follow-up: route `EditorInspector.svelte` preview thumbnail `<img>` through `BinaryTextureStore.objectUrlFor` so `/local/...` URIs never attempt a direct fetch (documented in the handoff Known-limitations section). Priority-1 file splits final gate complete: `museum-editor.svelte.ts` 3 640 → 2 554, `museum-editor.test.ts` 4 350 → 305 residual + four themed suites, `scene-codec.ts` 2 337 → 9-file directory (274-LOC barrel). Next slice: Phase 6 — brainstorm + spec first per the brainstorming skill. Latest handoff: [`../../agent-handoffs/phase-5.4.md`](../../agent-handoffs/phase-5.4.md). Final Priority-1 release handoff: [`../../agent-handoffs/2026-08-03-complete-priority-1-splits.md`](../../agent-handoffs/2026-08-03-complete-priority-1-splits.md).

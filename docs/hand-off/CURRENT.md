...
- **One action: open P7.6 — the museum-vocabulary scrub — the P7 facade-refactor umbrella's last increment (P7.1/P7.5/P7.2/P7.3 all shipped 2026-08-23; P7.4 shipped earlier via P1; the facade-decoupling line is complete).** P7.3 (Chopin defaults → explicit inputs): `document`/`rooms` now required `MuseumEditorStoreOptions`, camera-path/view `rooms` defaults removed (signature trap fixed — both `direction` + `rooms` required), `createLayoutPreviewState(layout, scene)` + `loadChopinLayoutPreview(state, layout)` parameterized, relic passes Chopin explicitly from `MuseumEditorApp.svelte`, ~30 test callsites migrated; both Chopin grep gates clean (zero symbols/imports in `src/lib/editor/` outside the relic shell); suite 1,988 green. **P7.6 readiness — the strings pass is a checklist, not a judgment call:** the [`P7.6 strings pre-inventory`](../plans/2026-08-23-P7.6-strings-pre-inventory.md) now stands at **516 lines / 522 occurrences** (P7.3 removed the `document`-seed comment; §1/§4/§5 updated) — R 340 / P 147 / T 35, all with targets, two-gate validated (§8), store/ assessed (§9). The three §3 flagged targets still await owner confirmation (`generator: 'museum-editor-5.4'`, the `x-museum-*` DnD MIME types, the `--museum-editor-fg` CSS token). Next: **P7.6** (lands last as designed), then P2 resumes. **Readiness (2026-08-22):** all pre-briefs are written — P7.1, P7.5, and P7.6 are marked `planned` in the umbrella, the 2026-08-19 file:line anchors are refreshed against the P8 S1–S6 delta, and P7.1 is pick-up-able today (13 verified write sites, three-tier write-class model, single-regex gate). **P7.6** was added 2026-08-22 with owner decisions recorded (drop-prefix scene vocabulary, relic keeps museum; hard-break format rename `.scenepack.zip` / `scene.json`); its §2 name map is collision-checked and its facade-test renames take the `editor-store-*` scheme (plain `editor-*` collides with existing module tests); §4 extended 2026-08-22 with a **camera/ + fields/ folder-placement commit group** (the 30-file camera surface + 3 generic widgets, `EditorCameraFovField` stays camera-side, test tree mirrored to `tests/lib/editor/camera/`) landing as pure `git mv`s before the rename series. Post-review (2026-08-22): §5 gate redesigned to two gates — identifier zero-match (incl. `MUSEUM_*` and mid-word `RuntimeMuseumScene`) + a tolerated prose/path list (relic imports, keep-list names, product prose), with `tests/lib/museum/**` excluded; §2 gained `MuseumState` → `RuntimeState`, the two `EditorProjectMenu` download filenames, and a functions/derived-names table (13 entries incl. `createMuseumEditorStore` → `createEditorStore`); `LayoutMuseumShell` added to the §3 keep-list (bench-comment prose); gate wording is now "zero outside §3 keep-list"; P7.6 routing row added to model-assessment.md. Remaining doc-before-pickup: none for P7.1; P7.2/P7.3/P7.5 briefs carry their own refresh notes (§P7.2–§P7.5); P7.6 needs only the committed P8+P7 delta beneath it. **Strings pre-inventory added 2026-08-23** — [`plans/2026-08-23-P7.6-strings-pre-inventory.md`](../plans/2026-08-23-P7.6-strings-pre-inventory.md): all 517 live-scope bare-`museum` lines / 523 occurrences pre-bucketed R 341 / P 147 / T 35 with targets (supersedes the §5 "~315" figure; §5 now points at it), so the strings pass is a checklist, not a judgment call. **Two-gate validation run the same day (§8):** identifier gate reconciled 1,145 occurrences / 49 distinct identifiers (43 §2 rename targets + 5 §3 keep-list names + `museumEditorEntryPlugin` — the one identifier-gate finding, resolved by adding it to the §3 keep-list since it is the keep-listed vite plugin's exported entry); bare gate re-verified 517/523 all bucketed, 0 uncovered. Adjudication surface for the strings pass: exactly four items (§8.3) — the plugin keep-list extension (decided) + the three §3 flagged targets. **store/ folder assessed the same day (§9):** no §4b sub-folder move (21 controllers, ~10.1k LOC, one internal import edge, 37 external importers; camera-domain 6/21 files but zero coupling → grouping is cosmetic; would-be `store/camera/` boundary recorded for post-P7.6 only); 12 museum identifiers across store/ all already in §2's map or §3 keep-list — zero new targets, gate blind-spot check clean, slice fully mechanical. P2 (Plan staging) resumes after P7; P3 stays last; P4/P5 unscheduled.

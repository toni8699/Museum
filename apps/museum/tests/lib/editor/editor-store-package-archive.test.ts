/**
 * `editor-store-package-archive.test.ts` — Phase 5.4 facade round-trip.
 *
 * RED coverage:
...
import { createEditorStore, EditorStore } from '$lib/editor/editor-store.svelte';
import {
	BinaryTextureStore,
...
describe('editor-store Phase 5.4 package-archive facade', () => {
	beforeEach(() => {
		__resetBinaryTextureStoreForTests();
	...
	describe('exportPackage', () => {
		it('returns ok with a Uint8Array + filename stamp matching "now"', async () => {
			...
			expect(result.filename).toMatch(/^scene-20260807-1830\.scenepack\.zip$/
);

			const map = unzipSync(result.zip);
			expect(map['manifest.json']).toBeDefined();
			expect(map['scene.json']).toBeDefined();
	...
	describe('round-trip: exportPackage → importPackageArchive', () => {
		it('restores byte-identical scene.json + primes BinaryTextureStore', async () => {
			const store = freshStore();
			await store.registerLocalFileTexture('Walnut', PNG_BYTES, 'image/png');
			...
			const carriedCanonical = new TextDecoder().decode(zipMap['scene.json']!);
			...
			// `scene.json`. This is the round-trip invariant.

...
import type { TextureVerifier } from '$lib/editor/texture-verifier';

describe('EditorStore Phase 5.2 texture facade', () => {
	...
	function createTextureStore(verifier: TextureVerifier = readyVerifier) {
		...
		return createEditorStore({ document, rooms: chopinRuntime.rooms, textureVerifier: verifier });
	}

	function registerTexture(store: EditorStore, uri: string) {

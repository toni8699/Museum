...
| **P20.S1** | 78 | Sol high | brief ready 2026-09-03, unregistered — Postgres registry + R2 streaming client + authenticated API + test seam; literal SQL, route table, hash-while-streaming upload, metadata + byte authz, pending/ready/failed states, and failure/rollback matrix are pinned; held by P19 live gate + owner-run R2 provisioning |

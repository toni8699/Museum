...
- Current delta: **P2.1 shipped uncommitted 2026-08-23 — footprint contract + room-aware passive Scene projection.** Added canonical `Asset.footprint` validation/metadata, live `SceneDocument` projection with effective uniform/session scale, room/entity yaw transforms, primitive eligibility, and passive dashed layer-6 render primitives. Main Scene Plan reads `store.document` + `store.rooms`; Camera Plan receives no Scene projection. P2.2 next. No commit.
...
- **One action: start P2.2 — Scene Plan-local mode + mode-aware footprint hit testing** ([`2026-08-18-P2-plan-staging.md`](../plans/2026-08-18-P2-plan-staging.md)). P2.1a–c are complete; P2.2 interaction gates remain recorded in the plan. P9 is closed and archived; P3 remains after P2. The existing uncommitted P7.6 delta remains untouched.

## Verification

- **1,995 tests green (1 skipped) · `svelte-check` 0 errors / 0 warnings ·
  ...
  (P2.1 complete, 2026-08-23; P2.1 added 6 projection/contract tests vs the
  1,989 P7.6 baseline —

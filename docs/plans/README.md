...
| P18 | backend provisioning — slice 4 of the ratified migration review: deploy `@portfolio/api` Fastify compute on Render, connect it to separately provisioned Neon Postgres through secret `DATABASE_URL`, and add process/database health checks with no persistence schema yet | **planned — ready to implement** | P17 | [2026-08-30-P18-backend-provisioning.md](2026-08-30-P18-backend-provisioning.md) |

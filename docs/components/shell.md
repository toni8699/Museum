...
Inspector · Unsequenced · Connections — Environment is read-only spatial
context); Scene 3D owns `Hierarchy | Assets`. Scene-only tabs, Assets, and
...
Timeline: collapsed ~36px / open ~220–360px; expansion state persists
verbatim and never auto-expands on a domain switch. Lanes: Guided Route ·
Camera Framing (no independent Pos/Target/FOV curves).

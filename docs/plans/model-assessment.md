...
| **P21.1** | 77 | Sol high | proposed (combined) — shared shell: tokens/primitives + Row 1 + layout-owned projectId-keyed session (A→B teardown) + Ribbon Zones A/B/C + 3-toolbar decomposition; broadest re-host risk in P21 (session ownership, capability matrix, Zone-A stability); umbrella-specified, no slice brief |
...
| **P21.5** | 45 (provisional) | Luna high | registered 2026-09-05 — UI polish pass; implementation spec filed (five slices + pinned single-ended gizmo declutter); presentation-only, no behavior change; lands before the P21 final acceptance gate and before P22 |

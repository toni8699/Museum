...
committed, S10.1.0–S10.1.7 landed uncommitted (1671 tests green), one S10.1.3
behavioral delta still open (standalone placement).
...
- **Workstream A — framing envelope (adopted model C′).** A data-model +
  sampler + authoring-UX feature that composes *inside* the landed Camera → 3D
  workspace. It changes `project.scene` serialization and the shared motion
  engine, so it is visitor-visible; the UI half mounts in S10.1 chrome.
- **Workstream B — Camera Plan + domain×view shell.** A **successor shell
  rework**, not an S10.1 increment. The graph-workspace design's domain×view
...
2. **The one open S10.1.3 item is the same mutator Camera Plan needs.**
   Standalone placement ("place → done, connect later") requires a
   "create free node, commit standalone" path; the graph-workspace doc's §4.3
   "create free node, commit standalone" row is that exact mutator. Land it
   **once** in S10.1 (B0 below) and both surfaces consume it.
3. **Plan already renders camera content top-down.** `plan-camera-projection.ts`
   projects paths, view cones, look targets, portal crossings, and timing
   labels onto the plan plane (layers 6–9 of `PlanRenderModel`), gated behind
   the Plan Tour toggle. Camera Plan is an **input-enabled variant of an
   existing render path**, not a new renderer or a new graph store.
4. **The connection-deletion invariant (graph doc §5.5) is already shipped.**
...
5. **The matrix rework touches freshly landed S10.1 files.** `H1EditorApp`,
   ...
   the graph doc's own §5.7 admits "several S10.1 plan assumptions." This is
...
**Scope.** Mounts in the landed Camera → 3D workspace: an envelope band on the
transition with enter/exit handles, one-gesture push-in, and the
"Full authored transition" command (`w = 1` escape hatch). Composes with two
S10.1 surfaces that already exist: the **View breakpoint inspector** (the
rotate-aim decision for view breakpoints is still open in S10.1.3 — the envelope
handles are a natural companion) and the **timeline** (S10.1.4 readout).
Include the FOV copy fix from the adopted model §7 ("wider / zoomed out" vs
"tighter / zoomed in") in the same UI pass so wording is standardized once.
...
## 4. Workstream B — Camera Plan + domain×view shell (from the graph-workspace design)
...
one remaining S10.1.3 behavioral delta). This is the graph-workspace doc's
...
### B1 — Domain×view shell matrix (successor slice, after S10.1 sign-off)

**Scope.** The deliberate shell rework from the graph doc §5: `EditorViewState`
gains a domain axis (`scene | camera`) with per-domain view state (`plan | 3d`,
fixed `[Plan | 3D]` order everywhere), the app bar switchers flip to
domain-primary, and the S10.1 shell shape (invariant 1) is **formally
superseded**. Preserve what S10.1 guarantees: one shared selection across a
domain's views (§5.3 — extends the S10.1.5 selection persistence), no new
store/document, context memory, Plan mutation-safety, and relic isolation.
Boot default: `Scene → 3D` per the review verdict (deferred, non-architectural).
...
**Scope.** The graph doc §4/§6: camera nodes at real world positions on the
...
|---|---|---|---|
| **Phase 1 — close S10.1** | B0 (standalone placement) | Last open S10.1.3 delta; also the mutator Camera Plan needs | 4/10 |
| **Phase 2 — engine (can land with or right after S10.1)** | A0 → A1 → A2 | Visitor-safe data + sampler; no shell dependency; S10.1's purity check already guards the bundle | 5/10 |
| **Phase 3 — framing UX** | A3 | Needs S10.1 chrome (Camera viewport, breakpoint inspector, timeline) + the S10.1.3 view-breakpoint rotate decision | 6/10 |
| **Phase 4 — successor shell** | B1 | Only after S10.1 sign-off; formally supersedes invariant 1 | 7/10 |
| **Phase 5 — surface** | B2 (+B3 assertions) | Depends on B1 mount + B0 mutator | 6/10 |

All five phases are **H1 camera-authoring scope** — classify them as H1
follow-on slices so they run **before** the plan-system renewal gate. If the
team instead classifies any phase as post-H1, it is hard-gated behind renewal.

## 6. Decisions to adjudicate before implementation

1. **Shell inversion (B1).** Approve superseding S10.1 invariant 1 after
   sign-off, or keep the current `Plan | 3D`-primary shell and mount Camera Plan
   differently (the graph doc's matrix is the reviewed answer — this is a
   ratification, not a re-litigation).
...
|---|---|---|---|
| B0 | Standalone placement mutator + workflow | — (in-flight S10.1) | graph doc §4.3/§6.2 + S10.1.3 |
...
| A3 | Envelope authoring UX + FOV copy | A0–A2, S10.1.3 rotate decision | adopted model §9.4 |
| B1 | Domain×view shell matrix | S10.1 sign-off | graph doc §5 |
| B2 | Camera Plan surface | B0, B1 | graph doc §4/§6.3 |
| B3 | Backdrop/visual-rule assertions | B2 | graph doc §4.4/§4.6/§5.2/§5.5 |

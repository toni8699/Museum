...
**Completed:** A0 LayoutDocument codec + B0 Chopin `rooms.ts` compiler + A1 pure line geometry/preview model/transaction stub + C0 `MuseumProject` codec + A2 read-only layout preview workspace + A2.1 rectangle/polygon drafting + A2.2 meter-scale editing and Chopin floor correction + A2.3 geometry-only opening authoring and numeric opening dimensions + A3 Bezier walls, arc-length openings, and derived arch profiles + A3.1 camera-style wall bend anchors, opening viz fix, and Plan opening drag + A4 layout objects, inspectors, and layout JSON I/O + A4.1 Layout Authoring Polish + B3 Room-Unit Relocate + B4 Runtime Dual-Read + B5 Serialized Project Runtime Cutover + G1 Shared Geometry Compiler + G2 Explicit Plan Render Boundary + G3 Graphics Performance Harness.

Full-track Phase 2 scene presets = deferred optional.

## Next slice

**G4 — Procedural architectural meshes.** Replace sampled wall-chord boxes progressively with meshes built from compiled wall sections: indexed topology, wall thickness/corners, opening side/lintel faces, arch profiles, stable UVs, material reuse, and disposal; merge/batch at room-and-material scale where the G3 baseline supports it. G3 is implemented in [`../plans/2026-08-13-graphics-g3-performance-harness.md`](../plans/2026-08-13-graphics-g3-performance-harness.md); roadmap: [`../plans/2026-08-13-graphics-architecture-roadmap.md`](../plans/2026-08-13-graphics-architecture-roadmap.md).
...
- G3 is shipped. The harness owns measurement, not optimization: seeded deterministic 10/100/1,000-room fixtures (30% auto-bezier, 2 openings/room, 3 objects/room), a pure Node tier (`compile`/`plan-render-build`/`hit-test`/`snap-query`/`compiled-memory`/`cache-key-cost`), a deterministic browser tier (initial render, pan/zoom and edit frame proxies, SVG node count, chord-box counts, `three-regen`), and a checked-in baseline with fail-closed budgets for the Chopin product scale only. 10/100/1,000-room tiers are comparison data, never enforced. Budget changes require a recorded reason in `g3-baseline.json`. The harness leaves the backlog (#1–#4, #10) unimplemented — each is now traced to a measured signal for G5.

## Out of scope this slice

Phase 2 Wall presets · G4+ graphics roadmap work · visitor rendering of layout objects · GLB import · new camera system · opening assets/frames · 3D room gizmos.
...
4. For G4, read the parent graphics roadmap plus architecture/persistence contracts. A0/B0/A1/C0/A2/A2.1/A2.2/A2.3/A3/A3.1/A4/A4.1/B3/B4/B5/G1/G2/G3 are shipped.

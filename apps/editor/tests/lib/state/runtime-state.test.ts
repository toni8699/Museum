...
import { createNavigationGraph, type RuntimeScene } from '$lib/content/scene';
...
describe('createRuntimeState', () => {
  it('resolves every transition against the injected graph', () => {
    ...
    const state = createRuntimeState(graph, 'custom-start');
  ...
  it('allows guided backtracking only after the previous room was visited', () => {
    const state = createRuntimeState(createNavigationGraph(scene), 'custom-start');
  ...
  it('keeps locked nodes non-navigable', () => {
    ...
    const state = createRuntimeState(createNavigationGraph(lockedScene), 'custom-start');

...
**Completed:** A0 LayoutDocument codec + B0 Chopin `rooms.ts` compiler + A1 pure line geometry/preview model/transaction stub.  
Full-track Phase 2 scene presets = deferred optional.

## Next slice

**C0** — `MuseumProject` codec (layout + scene), then **A2** first user-facing Plan/3D UI.
...
- A1 UI wiring deferred; no EditorViewport/facade/toolbar/history integration until A2/C0 decisions.
...
4. For C0, read only the C0 section in the foundation plan; for A2, read the A1 focused plan for the preview model contract. A0/B0/A1 are shipped.  
